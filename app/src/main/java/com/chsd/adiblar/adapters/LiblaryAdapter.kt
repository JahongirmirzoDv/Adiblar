...
class LiblaryAdapter(
    var fav: ArrayList<Writer>,
    var fragment: Fragment,
    var onpress: onPress
) :
    RecyclerView.Adapter<LiblaryAdapter.Vh>() {
    inner class Vh(var itemview: InnerItemBinding) : RecyclerView.ViewHolder(itemview.root) {
        ...
        fun Bind(writer: Writer, position: Int) {
            ...
            itemview.favorites.setOnClickListener {
                ...
                onpress.onclick(writer,position)
            }
    ...
    override fun onBindViewHolder(holder: Vh, position: Int) {
        holder.Bind(fav[position], position)
    }

    override fun getItemCount(): Int = fav.size
    interface onPress {
        fun onclick(writer: Writer,position: Int)
